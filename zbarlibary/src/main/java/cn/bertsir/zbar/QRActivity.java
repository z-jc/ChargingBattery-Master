...
import cn.bertsir.zbar.Qr.QRCodeUtil;
...
public class QRActivity extends Activity implements View.OnClickListener {
    ...
    private ScanCallback resultCallback = new ScanCallback() {
        ...
        public void onScanResult(String result) {
            ...
            QrManager.getInstance().getResultCallback().onScanSuccess(options.identify_type, result);

            if (options.identify_type == 1) {//连续扫码
            ...
            } else if (options.identify_type == 2) {//单次扫码
                finish();
            }
            /*finish();*/
        }
